...
function DirectionalArrows({ playerCount, mode, config, tableSize }: DirectionalArrowsProps) {
  ...
  const arrowRadius = (tableSize - PLAYER_SIZE) / 2 - 25;
    ...
    return (
      <>
        {Array.from({ length: playerCount }).map((_, index) => {
          ...
          return (
            <View key={index}>
              {/* Arrow to left neighbor */}
              <View
                style={{
                  position: 'absolute',
                  transform: [
                    { translateX: position.x - 15 },
                    { translateY: position.y },
